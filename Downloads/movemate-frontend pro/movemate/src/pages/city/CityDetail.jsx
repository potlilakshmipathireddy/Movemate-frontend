import { useState, useEffect } from 'react'
import { useParams, useNavigate } from 'react-router-dom'
import { motion } from 'framer-motion'
import { FiArrowLeft, FiStar, FiMapPin, FiCpu, FiSearch } from 'react-icons/fi'
import { cityService } from '../../api/axiosConfig'
import { SkeletonCard } from '../../components/common/Loaders'

const catEmoji = { RESTAURANT:'🍽️', HOSPITAL:'🏥', METRO:'🚇', BUS_STOP:'🚌', MARKET:'🛒', POLICE:'🚔', BANK:'🏦', TOURIST_SPOT:'🗺️', PARK:'🌳', TEMPLE:'🛕', MALL:'🏬', HOTEL:'🏨' }

export default function CityDetail() {
  const { city } = useParams()
  const navigate = useNavigate()
  const [overview, setOverview] = useState(null)
  const [attractions, setAttractions] = useState([])
  const [filtered, setFiltered] = useState([])
  const [aiPlaces, setAiPlaces] = useState([])
  const [searchQuery, setSearchQuery] = useState('')
  const [loading, setLoading] = useState(true)
  const [aiLoading, setAiLoading] = useState(false)
  const [activeCategory, setActiveCategory] = useState('')

  useEffect(() => {
    let isMounted = true

    Promise.all([
      cityService.getOverview(city).catch(() => null),
      cityService.getAttractions(city).catch(() => ({ data: [] })),
      fetch(`http://localhost:8080/api/city-explorer/ai/${city}`)
        .then(res => res.text())
        .then(text => (text ? JSON.parse(text) : null))
        .catch(() => null)
    ]).then(([ov, att, aiData]) => {
      if (!isMounted) return

      setOverview(ov?.data || null)
      const data = att?.data || []
      setAttractions(data)
      setFiltered(data)
      
      const parsedAi = typeof aiData === 'string' ? JSON.parse(aiData) : (aiData?.data || aiData)
      if (parsedAi) {
        if (Array.isArray(parsedAi.places)) {
          setAiPlaces(parsedAi.places)
        } else if (Array.isArray(parsedAi)) {
          setAiPlaces(parsedAi)
        }
      }
    }).finally(() => {
      if (isMounted) setLoading(false)
    })

    return () => { isMounted = false }
  }, [city])

  const filterCategory = async (cat) => {
    setActiveCategory(cat)
    if (!cat) { setFiltered(attractions); return }
    try {
      const res = await cityService.getByCategory(city, cat)
      setFiltered(res?.data || [])
    } catch { 
      setFiltered(attractions.filter(a => a.category === cat)) 
    }
  }

  // Handle AI Search when user submits a custom prompt query
  const handleAiSearch = async (e) => {
    if (e.key === 'Enter' && searchQuery.trim()) {
      e.preventDefault()
      setAiLoading(true)
      
      try {
        const url = `http://localhost:8080/api/city-explorer/ai/${city}?query=${encodeURIComponent(searchQuery)}`
        const response = await fetch(url)
        const text = await response.text()
        const parsed = text ? JSON.parse(text) : null
        
        let newPlaces = []
        if (parsed) {
          if (Array.isArray(parsed.places)) {
            newPlaces = parsed.places
          } else if (Array.isArray(parsed)) {
            newPlaces = parsed
          } else if (typeof parsed === 'string') {
            const innerParsed = JSON.parse(parsed)
            newPlaces = Array.isArray(innerParsed.places) ? innerParsed.places : []
          }
        }
        
        if (newPlaces.length > 0) {
          setAiPlaces(newPlaces)
        }
      } catch (err) {
        console.error("AI Search error:", err)
      } finally {
        setAiLoading(false)
      }
    }
  }

  const query = searchQuery.toLowerCase()
  const displayedFiltered = filtered.filter(att => 
    att.name?.toLowerCase().includes(query) || 
    att.description?.toLowerCase().includes(query)
  )

  // Directly display the dynamic AI places returned from the custom prompt query
  const displayedAiPlaces = aiPlaces

  const presentCategories = [...new Set(attractions.map(a => a.category))].filter(Boolean)

  return (
    <div style={{ paddingTop: 80, minHeight: '100vh', background: 'var(--dark)' }}>
      <div style={{ background: 'linear-gradient(135deg, var(--dark2), #1a1040)', padding: '60px 0 40px', borderBottom: '1px solid var(--border)' }}>
        <div className="container">
          <button onClick={() => navigate(-1)} className="btn-ghost-mm" style={{ marginBottom: 20, fontSize: '0.85rem' }}>
            <FiArrowLeft size={14} /> Back
          </button>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="section-eyebrow"><FiMapPin size={12} /> City Explorer</div>
            <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 900, fontSize: 'clamp(2rem, 4vw, 3rem)', marginBottom: 12 }}>
              <span className="gradient-text">{city}</span>
            </h2>
            {overview?.description && <p style={{ color: 'var(--text-muted)', maxWidth: 600, fontSize: '0.95rem', lineHeight: 1.7 }}>{overview.description}</p>}
          </motion.div>
        </div>
      </div>

      <div className="container" style={{ paddingTop: 40, paddingBottom: 60 }}>
        
        {/* --- Search Input Bar with Enter-to-AI-Search Support --- */}
        <div style={{ position: 'relative', marginBottom: 24 }}>
          <FiSearch size={18} style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} />
          <input 
            type="text" 
            placeholder="Search places or type custom queries (e.g. 'top 5 romantic spots') and press Enter..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            onKeyDown={handleAiSearch}
            style={{
              width: '100%',
              padding: '14px 16px 14px 48px',
              background: 'var(--dark2)',
              border: '1px solid var(--border)',
              borderRadius: '12px',
              color: '#fff',
              fontSize: '0.95rem',
              outline: 'none'
            }}
          />
        </div>

        {aiLoading && (
          <div style={{ textAlign: 'center', color: 'var(--primary)', marginBottom: 20, fontSize: '0.9rem' }}>
            ✨ AI is finding the best custom recommendations for you...
          </div>
        )}

        {presentCategories.length > 0 && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 32 }}>
            <button onClick={() => filterCategory('')}
              className={!activeCategory ? 'btn-primary-mm' : 'btn-ghost-mm'}
              style={{ padding: '8px 16px', fontSize: '0.82rem' }}>
              All
            </button>
            {presentCategories.map(cat => (
              <button key={cat} onClick={() => filterCategory(cat)}
                className={activeCategory === cat ? 'btn-primary-mm' : 'btn-ghost-mm'}
                style={{ padding: '8px 16px', fontSize: '0.82rem' }}>
                {catEmoji[cat] || '📍'} {cat.replace(/_/g,' ')}
              </button>
            ))}
          </div>
        )}

        {loading ? (
          <div className="row g-4">{Array.from({length:8}).map((_,i)=><div key={i} className="col-12 col-md-6 col-lg-4"><SkeletonCard /></div>)}</div>
        ) : displayedFiltered.length > 0 ? (
          <div className="row g-4">
            {displayedFiltered.map((att, i) => (
              <motion.div key={att.id || i} className="col-12 col-md-6 col-lg-4"
                initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                <div className="mm-card" style={{ padding: 24, height: '100%' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 14 }}>
                    <div style={{ fontSize: '2rem', flexShrink: 0 }}>{catEmoji[att.category] || '📍'}</div>
                    <div>
                      <h5 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: '1rem', marginBottom: 6 }}>{att.name}</h5>
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {att.category && <span className="mm-badge mm-badge-primary" style={{ fontSize: '0.68rem' }}>{att.category.replace(/_/g,' ')}</span>}
                        {att.rating && <span className="mm-badge mm-badge-warning" style={{ fontSize: '0.68rem' }}><FiStar size={10} /> {att.rating}</span>}
                      </div>
                    </div>
                  </div>
                  {att.description && <p style={{ color: 'var(--text-muted)', fontSize: '0.83rem', lineHeight: 1.6, marginBottom: 12 }}>{att.description}</p>}
                </div>
              </motion.div>
            ))}
          </div>
        ) : null}

        {/* --- AI Recommendations Section --- */}
        {displayedAiPlaces.length > 0 && (
          <div style={{ marginTop: 60 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24 }}>
              <FiCpu size={20} style={{ color: 'var(--primary)' }} />
              <h3 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, margin: 0 }}>AI Recommendations</h3>
            </div>
            <div className="row g-4">
              {displayedAiPlaces.map((place, i) => (
                <motion.div key={i} className="col-12 col-md-6 col-lg-4"
                  initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                  <div className="mm-card" style={{ padding: 24, height: '100%', border: '1px dashed var(--primary)' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 14 }}>
                      <div style={{ fontSize: '2rem', flexShrink: 0 }}>✨</div>
                      <div>
                        <h5 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: '1rem', marginBottom: 6 }}>{place.name}</h5>
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                          {place.category && <span className="mm-badge mm-badge-primary" style={{ fontSize: '0.68rem' }}>{place.category}</span>}
                        </div>
                      </div>
                    </div>
                    {place.description && <p style={{ color: 'var(--text-muted)', fontSize: '0.83rem', lineHeight: 1.6, marginBottom: 12 }}>{place.description}</p>}
                    {place.address && <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: 6 }}><FiMapPin size={11} /> {place.address}</div>}
                    {place.bestTimeToVisit && <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>📅 Best: {place.bestTimeToVisit}</div>}
                    {place.entryFee && <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>💰 Entry: {place.entryFee}</div>}
                  </div>
                </motion.div>
              ))}
            </div>
          </div>
        )}
      </div>
    </div>
  )
}